...
dados2|>
  rename(
  "Temperatura máxima doar " = maxat,  
  "Temperatura mínima do ar " = minat,
...
for (k in 1:10){
  ...
  reg <- lm(aux1 ~ aux_1 + aux_2 + aux_3 + aux_4 + aux_5 + aux_6 + aux_7 + aux_8 + aux_9, data = dados2)
...
colnames(dados2) <- aux
tab <- cbind(R2_j,VIF)
rownames(tab) <- c("Temperatura do ar máxima","Temperatura do ar mínima",
                  "Temperatura média do ar","Temperatura máxima do solo",
                  "Temperatura mínima do solo","Temperatura média do solo",
                  "Umidade relativa máxima", "Umidade relativa mínima",
